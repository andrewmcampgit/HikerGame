...
{
    public class NortheastChallengerWarehouse
    {
        public static Challenger GetChallenger()
        {
            ...
            Challenger northeastChallenger = challengerList[pickChallenger];

            challengerList.Remove(northeastChallenger);

            if (challengerList.Count == 0)
            {
                Console.WriteLine("There are no more challengers for this region. Try a different option.");

            }

            return northeastChallenger;
